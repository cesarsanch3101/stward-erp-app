...
const SalesOrderFormPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [cData, pData] = await Promise.all([getCustomers(), getProducts()]);
        setCustomers(cData || []);
        setProducts(pData || []);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!header.customer) return setError("Seleccione cliente.");
    setSaving(true);
    try {
      await createSalesOrder({ ...header, items });
      ...
      <Paper elevation={3} sx={{ p: 4 }}>
        ...
        <Box component="form" onSubmit={handleSubmit}>
          <Grid container spacing={2} mb={4}>
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth required>
                ...
                <Select value={header.customer} label="Cliente" onChange={(e) => setHeader({ ...header, customer: e.target.value })}>
                  {customers.map(c => <MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField fullWidth type="date" label="Fecha Envío" InputLabelProps={{ shrink: true }} 
                value={header.shipping_date} onChange={(e) => setHeader({ ...header, shipping_date: e.target.value })} 
              />
            </Grid>
          </Grid>
          ...
          <Box mt={3} display="flex" justifyContent="flex-end">
            <Button type="submit" variant="contained" disabled={saving}>{saving ? 'Guardando...' : 'Crear Orden'}</Button>
          </Box>
        </Box>
      </Paper>
    </Container>
